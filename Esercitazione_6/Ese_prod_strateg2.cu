...
//Funzioni core
__global__ void prodottoScalareGPU2(float *a, float *b, float *c, int n) {
...
int main(void) {
  ...
  double nBytes;          // size in byte di ciascun array
	...
	printf("***\t PRODOTTO SCALARE DI DUE VETTORI - 2 STRATEGIA \t***\n");
  ...
  prodottoScalareGPU2<<<gridDim, blockDim, blockDim.x>>>(A_device, B_device, C_device, n);
